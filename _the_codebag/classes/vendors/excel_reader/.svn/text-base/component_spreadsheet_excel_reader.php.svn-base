...
define('SPREADSHEET_EXCEL_READER_TYPE_NINETEENFOUR', 0x22); //34
...
class Spreadsheet_Excel_Reader
{
    ...
    //=================================
    //   METODOS AÑADIDOS POR EDUARDO
    //=================================
    public function get_numsheets(){return count($this->sheets);}
    public function get_numrows($iSheetIndex){ return $this->sheets[$iSheetIndex]["numRows"]; }
    public function get_numrows_by_sheetname($sSheetName)
    {
        $iSheetIndex = $this->get_sheet_index($sSheetName);
        return $this->get_numrows($iSheetIndex);
    }
    
    public function get_sheets_names()
    {
        $arSheetsNames = array();
        foreach($this->boundsheets as $arSheet)
            $arSheetsNames[] = $arSheet["name"];
        return $arSheetsNames;
    }   
    
    public function get_sheet_name($iSheetIndex)
    {
        foreach($this->boundsheets as $iIndex=>$arSheet)
            if($iSheetIndex==$iIndex) return $arSheet["name"];
        return "";
    }
    
    public function get_sheet_index($sSheetName)
    {
        //bug($this->boundsheets); die;
        foreach($this->boundsheets as $iKey =>$arSheet)
            if($arSheet["name"]==$sSheetName)
                return $iKey;
        return -1;
    }
  
    public function is_value_in_column($sSearch,$iSheetIndex,$iColumn)
    {
        $iNumRows = $this->sheets[$iSheetIndex]["numRows"];
        for($iNumRow=0; $iNumRow<=$iNumRows; $iNumRow++)
        {
            $sValue = $this->sheets[$iSheetIndex]["cells"][$iNumRow][$iColumn];
            //bug($sValue,"search $sSearch");
            if( trim(strtolower($sSearch)) == trim(strtolower($sValue)) ) return true;
        }
        return false;
    }
    
    public function is_value_in_columnn_by_sheetname($sSearch,$sSheetName,$iColumn)
    {
        $iSheetIndex = $this->get_sheet_index($sSheetName);
        return $this->is_value_in_column($sSearch, $iSheetIndex, $iColumn);
    }
    
    public function build_sql_insert($codCabecera,$arCampos,$sNombreTabla,$iSheetIndex,$iNumRow)
    {
        $arFieldNames = array("Code_Cabecera");
        $arFieldValues = array($codCabecera);
        foreach($arCampos as $iSheetColumn=>$sNombreCampo)
        {
            $arFieldNames[] = $sNombreCampo;
            $arFieldValues[] = $this->sheets[$iSheetIndex]["cells"][$iNumRow][$iSheetColumn];
        }
        $sValues = implode("','",$arFieldValues);
        $sValues = "'$sValues'";
        $sSQLInsert = "INSERT INTO $sNombreTabla (".implode(",",$arFieldNames).")";
        $sSQLInsert .= "VALUES($sValues)";
        return $sSQLInsert;
    }
    //=================================
    // FIN METODOS AÑADIDOS POR EDUARDO
    //=================================    
...
//exit;
    ...
    */
        ...
        if($res === false)
        {
            ...
            {
                $this->_iserror = true;
                $this->_message = "The filename $sFileName is not readable";
                return;
            // bad file
...
//        var_dump($res);
    ...
    function _parse()
    {
        ...
        //codigo hexadecimal
        //ord devuelve el valor ascii del primer caracter de un string
        //http://php.net/manual/en/language.operators.bitwise.php operador <<
        ...
        while($code != SPREADSHEET_EXCEL_READER_TYPE_EOF)
        {
            switch ($code)
            {
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
                    ...
                    //bug($formatString,"formatstring, indexcode: $indexCode");
...
//            $r = &$this->file->nextRecord();
    ...
    {
        //"numValue:41820 - string:01/07/2014 - raw:1404172800"
        if($numValue > 1)
        {
            //1904 -> 24107 sino 25569
            $utcDays = $numValue - ($this->nineteenFour ? SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS1904 : SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS);
            //Comentado por Edu. No se pq motivo le suman 1 día. Esto me estaba generando error
            //$utcValue = round(($utcDays+1) * SPREADSHEET_EXCEL_READER_MSINADAY);
            $utcValue = round(($utcDays) * SPREADSHEET_EXCEL_READER_MSINADAY);
            //bug("nineteenFour: $this->nineteenFour -  utcDays:$utcDays - utcValue:$utcValue - curformat:$this->curformat");
        ...
        //bug("numValue:".$numValue." - string:".$string." - raw:".$raw);
