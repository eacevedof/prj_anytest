function MainHelper()
{
    var sTableId = "tblProductos";
    var iNumRows = 0;
    //var arObjRows = [];
    var arRowsIds = [];
    var arRowsFound = [];
    //como object pq asi permite usar keys asociativos
    var arTableProducts = new Object();
    
    var txtBusquedaGeneral = null;//jQuery("#txtBusquedaProducto");
    ...
    var __construct = function()
    {
        ...
        if(oTable!=null)
        {
            txtBusquedaGeneral = document.getElementById("txtBusquedaGeneral");
            var arObjRows = oTable.rows;//.length
            //bug(arObjRows,"arObjRows");
            iNumRows = arObjRows.length;
            var sRowId="";
            //var sTdId="";
            var oRow=null;
            var iNumCells=0;var arObjCells = null;
            var oCell=null;var sFieldName="";
            var objRowCells=new Object();
            //http://www.fperezp.com/blog/2011/03/12/arrays-asociativos-en-javascript/
            if(iNumRows>0)
            {
                iNumCells = arObjRows[0].cells.length;
                for(var i=0; i<iNumRows; i++)
                {
                    oRow = arObjRows[i];
                    sRowId = oRow.id;
                    arObjCells = oRow.cells;
                    for(var j=0; j<iNumCells; j++)
                    {
                        oCell = arObjCells[j];
                        //sTdId = oCell.id.toString();
                        sFieldName = oCell.getAttribute("fieldname");
                        objRowCells[sFieldName] = get_text_from_textnodes(oCell).toLowerCase();
                    }
                    arRowsIds.push(sRowId);
                    arTableProducts[sRowId]=objRowCells;
                    objRowCells = new Object();
                }
            }    
            arObjRows = null;
        }
